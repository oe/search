...
async function getImageAverageColor(imageSrc: string) {
  ...
  const calculatedPointCount = Math.min(pixelCount, MAX_POINT_COUNT)
  // step to calculate average color, to avoid too many calculations
  const step = Math.floor(pixelCount / calculatedPointCount)
  ...
  averRgba.r = Math.floor(averRgba.r / calculatedPointCount)
  averRgba.g = Math.floor(averRgba.g / calculatedPointCount)
  averRgba.b = Math.floor(averRgba.b / calculatedPointCount)
  averRgba.a = Math.floor(averRgba.a / calculatedPointCount)
